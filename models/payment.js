...
exports.announcePayment = async (value) => {
  const table_QLTT = new pgp.helpers.TableName({
    table: QuanLyThanhToan,
    schema: schema,
  });
  ...
  const qStr_loan = pgp.as.format(
    'SELECT "SoDuNo" FROM $1 WHERE "NguoiLienQuan" = $2',
    [table_SDN, value]
  );
  try {
...
exports.getAccount = async (value) => {
  ...
  const qStr = pgp.as.format(
    `SELECT * FROM $1 WHERE "Username" = '${value}'`,
    table
  );
  try {
    const res = await db.one(qStr);
...
exports.getLoan = async (id) => {
  ...
  const qStr = pgp.as.format(
    `SELECT * FROM $1 WHERE "NguoiLienQuan" = '${id}'`,
    SoDuNo
  );
  try {
    const res = await db.any(qStr);
...
exports.getLevel = async () => {
  const QLTT = new pgp.helpers.TableName({
    table: "QuanLyThanhToan",
    schema: schema,
  });
...
exports.listPayment = async () => {
  const SDN_table = new pgp.helpers.TableName({ table: SDN, schema: schema });
  const NLQ_table = new pgp.helpers.TableName({ table: NLQ, schema: schema });
  const qStr = pgp.as.format(
    `SELECT "NLQ"."MaNguoiLienQuan","NLQ"."HoTen","SDN"."SoDuNo"
  FROM $1 "NLQ" INNER JOIN $2 "SDN" ON "SDN"."NguoiLienQuan" = "NLQ"."MaNguoiLienQuan"`,
    [NLQ_table, SDN_table]
  );
  try {
    const res = await db.any(qStr);
...
exports.getLimit = async (tbName) => {
  const table = new pgp.helpers.TableName({ table: tbName, schema: schema });
  const qStr = pgp.as.format(
    `SELECT *FROM $1 ORDER BY "MaQLTT" DESC LIMIT 1`,table);
  try {
    const res = await db.one(qStr);
    return res;
  } catch (error) {
